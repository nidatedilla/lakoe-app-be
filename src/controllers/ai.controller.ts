...
const extractProductName = (message: string): string | null => {
  const commonWords = ['jika', 'saya', 'ingin', 'menjual', 'berapa', 'kira', 'harga', 'yang', 'cocok'];
  ...
  try {
    ...
    if (!isProductRelatedMessage(message)) {
      ...
      try {
        ...
        return res.json({error: 'Error processing request', details: error  });
      }
    }
    
    if (!productName) {
      return res.json({ reply: "Mohon berikan nama produk yang valid untuk mendapatkan rekomendasi harga." });
    }
    ...
    if (products.length > 0) {
      ...
      return res.json({ reply: `Produk '${product.name}' sudah ada dalam database. Harga yang sesuai adalah Rp${product.price}.` });
    }
